...
class MasterKeyManager(private val context: Context) {
     ...
     *
     * `@Synchronized`: два параллельных ПЕРВЫХ вызова (напр. ключ БД и ключ аудио) могли бы
     * перемежить пересоздание ключа-обёртки и запись blob — данные оказались бы зашифрованы
     * мастером, чьей обёртки больше нет на диске (невосстановимая потеря). Сериализуем весь путь.
     */
    @Synchronized
    ...
    private fun prepareWorkingWrapKey() {
        ...
        // Проба могла провалиться не из-за прошивки, а из-за ТЕКУЩЕЙ блокировки устройства
        // (первый доступ к ключу из фона). Это транзиентно: НЕ понижаем защиту фолбэком,
        // а сигналим «заблокировано» — вызывающий повторит после разблокировки.
